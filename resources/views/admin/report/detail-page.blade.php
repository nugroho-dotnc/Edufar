<x-app-layout>
    ...
    <div class="h-[43rem] md:h-[38rem] overflow-y-scroll">
        <div class="max-w-7xl mx-auto md:my-6 grid grid-cols-3 md:h-[38rem] h-[45.15rem] gap-2 md:gap-4 relative">
            ...
            <div class="col-span-3  shadow-md shadow-gray-200 p-4">
                <form method="POST" enctype="multipart/form-data" action="{{route('admin.response.store', ['id'=>$report->id])}}" >
                    @csrf
                    <label class="form-control">
                        <div class="label">
                            <span class="label-text text-gray-500">Administrator response</span>
                        </div>
                        <textarea class="textarea outline-none border-gray-200 text-gray-500 md:h-24  md:max-h-24 h-32 max-h-32 overflow-y-scroll bg-white " name="description" placeholder="Bio">
                            {{$response->description ?? "none of response here"}}
                        </textarea>
                    </label>
                    <div class="flex justify-end mt-4 gap-2">
                        ...
                        <x-process-button :report="$report"/>
                    </div>
                </form>
            </div>
        </div>
